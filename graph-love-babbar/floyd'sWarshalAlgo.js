// Floyd Warshall
// Difficulty: MediumAccuracy: 32.89%Submissions: 201K+Points: 4Average Time: 15m
...
// do n iteration in dp matrix

// from every source check the distance of every node if its possible or min then 
// update the distance

// the last matrix will be the shortest path from every node to every node

// floyd warshall algo is used to find shortest path from every node to every another node

// you can also detect cycle with this algo
// if the distance on any node to itself is less than 0 then there is a cycle
// dist[i][i] < 0 then there is a cycle
// because initally all the distance will be 0 at dist[i][i]
        ...
        for(let k=0;k<dist.length;k++){
            for(let i=0;i<dist.length;i++){
                ...
                for(let j=0;j<dist[i].length;j++){
                    ...
                    if(dist[i][k]!= 10**8 && dist[k][j]!= 10**8){
                    let newDist = dist[i][k]+dist[k][j]
                    
                    if(newDist  < dist[i][j] && dist[i]){
                        dist[i][j] = newDist
                    }
                    }
